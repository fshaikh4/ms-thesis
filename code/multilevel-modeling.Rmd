...
for (chosen_year in 2015:2023) {
    ...
    chosen_chr_raw_data <- read_excel(
        ...
        mutate(
            ...
            `Social & Economic Factors` = as.numeric(`Social & Economic Factors`),
...
combined_edvisits_df <- combined_edvisits_df %>% # need to be separate from above for some reason
    rename(
...
model_3_rates <- combined_edvisits_df %>%
    ...
    mutate(Year.adj = Year - 2015) %>% # for interpretation
    lme(
        fixed = nonheroin_opioid_rate_per_100K ~ Year.adj,
        random = ~ Year.adj | Locality
    )
...
model_4_counts <- combined_edvisits_df %>%
    ...
    mutate(Year.adj = Year - 2015) %>% # for interpretation
    lme(
        fixed = combined ~ Year.adj + zHB + zCC + zSEF + zPE,
        random = ~ Year.adj | Locality
    )
...
model_4_rates <- combined_edvisits_df %>%
    ...
    mutate(Year.adj = Year - 2015) %>% # for interpretation
    lme(
        fixed = combined_rate_per_100K ~ Year.adj + zHB + zCC + zSEF + zPE,
        random = ~ Year.adj | Locality
    )
...
So far, in my efforts, I have had issues replicating the results from publication. As a last effort, I am now trying to use the numbers obtained directly from the VDH for ED visit rates per 100K population (rather than via my own calculation combining 2 data sources). I perform these tasks below:

```{r read-original-data}
original_data_fname <-
    "extract-rates-per-100k-pop.xlsx"
original_data_fullpath <-
    paste0(data_dir_path, original_data_fname)

# read-in the data
original_data_df <- read_excel( # original analyses were nonheroin-opioid only,
                                # so we select the appropriate sheet below
    path = original_data_fullpath,
    sheet = excel_sheets(original_data_fullpath)[1]
    ) %>%
    slice(-1) %>% # remove first non-data row
    select("OPIOID", paste0("...", ((0:(2023 - 2015)) * 4) + 5)) %>%
    # the above formula helps us access the autogenerated column #s of our
    # desired measure: avg monthly overdose rates per 100K population
    rename("Locality" = "OPIOID") %>% # rename OPIOID col to Locality
    mutate(Locality =
        str_to_upper(str_replace_all(Locality, "[^a-zA-Z0-9 ]", ""))
    ) %>%
    # the above line is to remove special characters (i.e., double-dagger) and
    # uppercase all locality names to avoid name case issues during merge
    rename_with(
        .fn = function(original_colname) {
            ...
            new_colname <- as.character(2015 + (original_colname_num - 5) / 4)

            return(new_colname)
        },
        .cols = 2:last_col() # ignore OPIOID col
    ) %>%
    pivot_longer( # make data long format
        cols = as.character(2015:2023),
        names_to = "Year", # ensure this is updated to match the data timeframe
        values_to = "pub_rate_per_100K" # Avg Monthly Opioid ED Visit Rate per
                                        # 100K Population, measure used in pub
    ) %>%
    mutate(
            Year = as.numeric(Year), # convert Year back to numeric for merge
            pub_rate_per_100K = as.numeric(pub_rate_per_100K) # was character
    )

# merge-in other relevant data from combined_edvisits_df
replication_df <- combined_edvisits_df %>%
    left_join(original_data_df) %>%
    mutate(
        annual_pub_rate_per_100K = 12 * pub_rate_per_100K,
        error_from_pub_rate = annual_pub_rate_per_100K - nonheroin_opioid_rate_per_100K,
        rel_error_from_pub_rate = error_from_pub_rate / annual_pub_rate_per_100K
    )
```

If we are interested, we can look at the differences between our calculated data and the VDH data for annual rates per 100K:

```{r present-rates-calc-vs-VDH}
replication_df %>%
    select(Locality, Year, nonheroin_opioid_rate_per_100K, annual_pub_rate_per_100K, error_from_pub_rate, rel_error_from_pub_rate) %>%
    knitr::kable(style = "html")
```

And below, we run models on our newly-created `replication_df` to generate results from the original data used in the publication.

```{r run-models-for-original-data}
# model 1: unconditional (growth)
model_1_pub <- replication_df %>%
    filter(Locality != "VIRGINIA") %>%
    filter(Year %in% 2016:2021) %>% # match w/ publication
    lme( # nlme implementation
        fixed = pub_rate_per_100K ~ 1,
        random = ~ 1 | Locality
    )
    # lmer( # lme4 implementation
    #     formula = pub_rate_per_100K ~ (1 | Locality),
    # )

summary(model_1_pub)

# model 2: fixed growth
model_2_pub <- replication_df %>%
    filter(Locality != "VIRGINIA") %>%
    filter(Year %in% 2016:2021) %>% # match w/ publication
    mutate(Year.adj = Year - 2015) %>% # for interpretation
    lme(
        fixed = pub_rate_per_100K ~ Year.adj,
        random = ~ 1 | Locality
    )
    # lmer( # lme4 implementation
    #     formula = pub_rate_per_100K ~ Year.adj + (1 | Locality),
    # )

summary(model_2_pub)

# model 3: random growth
model_3_pub <- replication_df %>%
    filter(Locality != "VIRGINIA") %>%
    filter(Year %in% 2016:2021) %>% # match w/ publication
    mutate(Year.adj = Year - 2015) %>% # for interpretation
    lme(
    ...
    # lmer( # lme4 implementation
    #     formula = pub_rate_per_100K ~ (Year.Adj | Locality) + (1 | Locality),
    # )

summary(model_3_pub)

# model 4: random growth w/ TVCs
model_4_pub <- replication_df %>%
    filter(Locality != "VIRGINIA") %>%
    filter(Year %in% 2016:2021) %>% # match w/ publication
    na.omit() %>% # 1 set of z-scores is NA
    mutate(Year.adj = Year - 2015) %>% # for interpretation
    lme(
        fixed = pub_rate_per_100K ~ Year.adj + zHB + zCC + zSEF + zPE,
        random = ~ Year.adj | Locality
    )
    # lmer( # lme4 implementation
    #      formula = pub_rate_per_100K ~ zHB + zCC + zSEF + zPE 
    #         + (Year.adj | Locality) + (1 | Locality),
    # )
