...
mutate(combined_rate = nonheroin_rate + heroin_rate) %>%
pivot_longer(
...
```{r recode-remove-replicate-localities, eval=F}
...
### Data exclusion: Localities unable to self-compare before 2021
...
```{r recode-remove-quality-issues-localities, eval=F}
...
```{r summarize-missingness, results="asis"}
...
locality_rates_plotter <- function(locality, measure) {
    ...
    ggplot(aes(x = date, y = measure_value, group = Locality)) +
        geom_line() +
        geom_point() +
        theme_classic() +
...
locality_rates_plotter("VIRGINIA", "combined_rate")

# plot each county (separately) [COMMENTED-OUT FOR NOW]
# monthly_rates_combined_long[["Locality"]] %>% unique() %>%
