...
alldata_summarized_df <- alldata_df %>%
    ...
    filter(
        !all(is.na(combined)), # remove county when ALL its rows are NA
        !all(is.na(Deaths)) # remove county when ALL its rows are NA
    ) %>%
    summarize( # calculate slopes/intercepts
        ...
        # edvisits_pop_coeff = coef(lm(combined ~ Year.adj))[[3]],
        deaths_timecoeff = coef(lm(Deaths ~ Year.adj))[[2]],
        deaths_intercept = coef(lm(Deaths ~ Year.adj))[[1]],
        # deaths_pop_coeff = coef(lm(Deaths ~ Year.adj))[[3]]
    ) %>%
