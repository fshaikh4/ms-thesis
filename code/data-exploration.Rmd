...
alldata_summarized_df <- alldata_df %>%
    ...
    summarize( # calculate slopes/intercepts
        edvisits_timecoeff = coef(lm(combined ~ Year))[[2]],
        edvisits_intercept = coef(lm(combined ~ Year))[[1]],
        deaths_timecoeff = coef(lm(Deaths ~ Year))[[2]],
        deaths_intercept = coef(lm(Deaths ~ Year))[[1]]
    ) %>%
    right_join(va_counties, by = c("Locality" = "NAMELSAD")) # merge-back
                                                             # removed counties

# plot slopes/intercepts for edvisits data
alldata_summarized_df %>% # slopes
    ggplot() +
